...
# files = list(glob(path_true + '/*.jpg'))
for fn in sorted(files):
    name = basename(str(fn))
    names.append(name)

    # img = np.array(Image.fromarray(imread(str(fn))).resize(size = (256, 256)))
    # img = Image.open(str(fn))
    # img = img.resize((256, 256))
    img_gt = (imread(str(fn)) / 255.0).astype(np.float32)
    # img_gt = (np.array(img)/255.0).astype(np.float32)
    img_pred = (imread(path_pred + '/' + basename(str(fn))) / 255.0).astype(np.float32)
    # img_pred = (np.array(Image.open(path_pred + '/' + basename(str(fn))))/255.0).astype(np.float32)

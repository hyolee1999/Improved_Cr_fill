...
class gen_conv(nn.Conv2d):
    def __init__(self, cin, cout, ksize, stride=1, rate=1, activation=nn.ELU()):
        ...
        if cout == 3 or activation is None:
            ...
            super(gen_conv, self).__init__(in_channels=cin, out_channels=cout//2, kernel_size=ksize, stride=stride, padding=p, dilation=rate, groups=1, bias=True)
            self.mask_conv2d1 = nn.Conv2d(in_channels=cin, out_channels=cin, kernel_size=ksize, stride=stride, padding=p, dilation=rate, groups=cin, bias=True)
            self.mask_conv2d2 = nn.Conv2d(in_channels=cin, out_channels=cout//2, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=True)
        self.cout = cout
        self.activation = activation

    def forward(self, input):
        x = super(gen_conv, self).forward(input)
        if self.cout == 3 or self.activation is None:
            return x
        mask = self.mask_conv2d1(input)
...
class gen_deconv(gen_conv):
    def __init__(self, cin, cout):
        ...
        Args:
            ...
            ksize: Kernel size.
        """
        super(gen_deconv, self).__init__(cin, cout*4, ksize=3)
